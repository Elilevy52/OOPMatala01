		...
		do {
			...
			case 6:
				...
				if (newChoice3 == 'y' || newChoice3 == 'Y') {
					manage.printAllQuestions();
					System.out.println("How many questions you want in your exam? ");
					amountOfQuestions = input.nextInt();
					manage.setSize(amountOfQuestions);
				} else if (newChoice3 == 'n' || newChoice3 == 'N') {
					innerOP = input.nextInt();
					innerOP = -1;
					input.nextLine();
					break;
					...
					for (int i = 0; i < amountOfQuestions; i++) {
						int questionNum1 = input.nextInt();
						input.nextLine();
						if (!manage.checkInstanceofQuestion(questionNum1)) {
							manage.addQuestionToManualExam(questionNum1, null);
						} else {
							...
							manage.getMultipeChoiseAnswer(questionNum1);
							System.out.println("Please select the answers you want: ");
							for (int j = 0; j < answersArray.length; j++) {
								answersArray[j] = input.nextInt();
							}
							manage.addQuestionToManualExam(questionNum1, answersArray);
						}
